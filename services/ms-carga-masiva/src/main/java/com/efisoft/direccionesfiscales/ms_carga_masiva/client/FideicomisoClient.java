package com.efisoft.direccionesfiscales.ms_carga_masiva.client;

import com.efisoft.direccionesfiscales.ms_carga_masiva.dto.NombreDTO;
import com.efisoft.direccionesfiscales.ms_carga_masiva.dto.RfcDTO;
...
public class FideicomisoClient {
    ...
    public Optional<String> getRfc(String tipoParticipante, int numContrato, int numParticipante) {
        try {
            var dto = restClient.get()
                .uri("/api/v1/fideicomisos/{numContrato}/participantes/{tipoParticipante}/{numParticipante}/rfc",
                    numContrato, tipoParticipante, numParticipante)
                .retrieve()
                .body(RfcDTO.class);
            return dto != null ? Optional.ofNullable(dto.getRfc()) : Optional.empty();
        } catch (RestClientResponseException e) {
            log.warn("RFC lookup failed for contrato={} tipo={} participante={}: HTTP {}",
                numContrato, tipoParticipante, numParticipante, e.getStatusCode());
            return Optional.empty();
        } catch (ResourceAccessException e) {
            log.error("RFC lookup connection error for contrato={} tipo={} participante={}: {}",
                numContrato, tipoParticipante, numParticipante, e.getMessage());
            return Optional.empty();
        }
    }

    public Optional<String> getNombre(String tipoParticipante, int numContrato, int numParticipante) {
        try {
            var dto = restClient.get()
